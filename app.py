...
st.title("🧠 费曼三层解释器")
st.caption("输入任何概念，AI 用三种深度为你解释 | Day 10 · 选项卡交互")
...
with st.sidebar:
    ...
    st.divider()
    st.header("🌐 添加网页")
    web_url = st.text_input("粘贴文章URL", placeholder="https://...", key="web_url")
    if st.button("⬇️ 抓取并保存", use_container_width=True):
        if web_url.strip():
            from utils import fetch_url
            result = fetch_url(web_url.strip())
            if result:
                text, title = result
                safe_name = "".join(c for c in title if c.isalnum() or c in " _-")[:50] or "webpage"
                filepath = os.path.join("docs", f"{safe_name}.txt")
                with open(filepath, "w", encoding="utf-8") as f:
                    f.write(text)
                st.success(f"✅ 已保存到 docs/{safe_name}.txt")
                st.info("💡 在终端运行 `python ingest.py` 重建知识库即可生效")
            else:
                st.error("❌ 抓取失败，请检查URL或网络")
        else:
            st.warning("请输入URL")
...
CITE_RULE = """
【溯源规则 · 所有层级通用】
如果回答中使用了参考资料里的信息，必须在句末标注来源编号，格式为[来源N]。
例如："GRPO通过群组归一化来减少方差[来源1]，与传统PPO方法相比更高效[来源3]。"
如果回答没有用到任何参考资料，或参考资料不足以回答问题，可以不标注。
"""
...
def retrieve_context(question: str, k: int, bm25_w: float):
    ...
    for i, (_, info) in enumerate(sorted_items[:k]):
        ...
        parts.append(
            f"[来源{src_id}] 来源：{info['source']}\n{info['content']}"
        )

    context = "\n\n---\n\n".join(parts) if parts else ""
    return context, scored_docs, footnotes
    ...
    if not api_key:
        ...
        if use_rag and vectordb:
            with st.spinner("🔍 检索知识库..."):
                context, scored_docs, footnotes = retrieve_context(question, top_k, bm25_weight)
            ...
            for label, (sys_prompt, user_prefix) in PROMPTS.items():
                ...
                try:
                    for token in explain_layer_stream(
                        ...
                        full_text += token
                    results[label] = full_text
                except Exception as e:
                    results[label] = f"❌ 出错了：{e}"

        # ── 选项卡展示 ──
        tabs = st.tabs(["🐣 12岁孩子", "📘 技术同事", "🧪 领域专家", "📊 三栏对比"])

        # 单层标签
        for tab, (label, _) in zip(tabs[:3], PROMPTS.items()):
            with tab:
                if label in results:
                    st.markdown(results[label])

        # 对比标签
        with tabs[3]:
            col1, col2, col3 = st.columns(3)
            for col, (label, _) in zip([col1, col2, col3], PROMPTS.items()):
                with col:
                    st.subheader(label)
                    if label in results:
                        st.markdown(results[label])
        ...
        # ── 脚注溯源（Day 9 新增） ──
        if footnotes:
            with st.expander("📖 回答溯源（点击查看每条引用的原文）", expanded=False):
                for src_id, fn in footnotes.items():
                    st.markdown(f"**[来源{src_id}]** — *{fn['source']}*")
                    st.caption(fn["preview"] + "...")
                    st.divider()
...
st.divider()
st.caption("Day 10 · 选项卡交互 · 费曼项目 · Away")
